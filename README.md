ver.0.99.26
  - переробив функціонал форми для Народного кориспондента(не до кінця)
  - сповіщення для публікацій Народного кориспондента, які чекають на підтвердження (functions.php)
